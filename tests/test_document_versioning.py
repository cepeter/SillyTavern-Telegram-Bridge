...
class DocumentVersioningTests(unittest.TestCase):
    ...
    def test_same_filename_creates_new_active_version(self):
        status1, chunks1 = rt.add_data_bank_document(
            self.db, "chat", "notes.txt", b"alpha old version"
        )
        status2, chunks2 = rt.add_data_bank_document(
        ...
        versions = rt.data_bank_document_versions(self.db, "chat", "notes.txt")
        self.assertEqual([row[1] for row in versions], [2, 1])
        self.assertEqual([row[2] for row in versions], [1, 0])

        active = rt.data_bank_documents(self.db, "chat")
    ...
    def test_exact_content_reupload_stays_duplicate(self):
        ...
        self.assertEqual(
            rt.add_data_bank_document(self.db, "chat", "notes.txt", payload)[0],
            "added",
        )
        self.assertEqual(
            rt.add_data_bank_document(self.db, "chat", "renamed.txt", payload)[0],
            "duplicate",
        )
    ...
    def test_retrieval_uses_only_active_version_and_rollback_is_atomic(self):
        ...
        current = rt.retrieve_data_bank(self.db, "chat", "spaceship")
        old_hidden = rt.retrieve_data_bank(self.db, "chat", "dragon")
        self.assertTrue(current)
        self.assertEqual(old_hidden, [])

        self.assertTrue(rt.activate_data_bank_version(self.db, "chat", "story.txt", 1))
        old_visible = rt.retrieve_data_bank(self.db, "chat", "dragon")
        new_hidden = rt.retrieve_data_bank(self.db, "chat", "spaceship")
    ...
    def test_activate_unknown_version_does_not_change_active_version(self):
        rt.add_data_bank_document(self.db, "chat", "notes.txt", b"version one")
        rt.add_data_bank_document(self.db, "chat", "notes.txt", b"version two")
        self.assertFalse(rt.activate_data_bank_version(self.db, "chat", "notes.txt", 99))
        versions = rt.data_bank_document_versions(self.db, "chat", "notes.txt")
