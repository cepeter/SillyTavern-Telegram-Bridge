from application_test_setup import make_test_conversation_service, make_test_provider_port, make_test_input_flow_service
...
class SessionNamingTests(unittest.TestCase):
    ...
    def test_standard_session_is_created_only_after_valid_name(self):
        ...
        self.assertTrue(make_test_input_flow_service().handle_pending(self.db, "token", "chat", self.session, "  Project   Alpha  ", operation_id=42, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, self.session["session_id"])))
    ...
    def test_status_displays_custom_session_title_with_technical_id(self):
        ...
        try:
            make_test_conversation_service().process_message(self.db, "token", "key", _m_memory_curator.DEFAULT_MODEL, {}, "chat", "/status", services=make_test_application_services(memory=make_test_memory_service()))
        finally:
    ...
    def test_invalid_name_reprompts_without_creating_session(self):
        _m_session_naming.start_session_name_input(self.db, "token", "chat", self.session, group_service=self.group)
        self.assertTrue(make_test_input_flow_service().handle_pending(self.db, "token", "chat", self.session, "/bad", operation_id=43, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, self.session["session_id"])))
        self.assertEqual(len(_m_panel_callback_routes.list_sessions(self.db, "chat")), 1)
    ...
    def test_cancel_leaves_no_empty_session_or_pending_character(self):
        ...
        _m_session_naming.start_session_name_input(self.db, "token", "chat", self.session, group_service=self.group)
        self.assertTrue(make_test_input_flow_service().handle_pending(self.db, "token", "chat", self.session, "/cancel", operation_id=44, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, self.session["session_id"])))
        self.assertEqual(len(_m_panel_callback_routes.list_sessions(self.db, "chat")), 1)
    ...
    def test_cancel_with_bot_mention_leaves_no_empty_session(self):
        ...
        self.assertTrue(make_test_input_flow_service().handle_pending(self.db, "token", "chat|topic:7", self.session, "/cancel@SillyTavernPunzmeBot", operation_id=47, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, self.session["session_id"])))
    ...
    def test_character_chain_applies_character_after_name(self):
        ...
        try:
            make_test_input_flow_service().handle_pending(self.db, "token", "chat", self.session, "Chosen Story", operation_id=45, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, self.session["session_id"]))
        finally:
    ...
    def test_group_session_waits_for_name_then_opens_character_stage(self):
        ...
        try:
            ...
            make_test_input_flow_service().handle_pending(self.db, "token", chat_id, session, "Mystery Team", operation_id=46, group_service=self.group, provider_port=make_test_provider_port(), memory_service=make_test_memory_service(), persona_service=make_test_persona_service(), request_context=make_test_request_context(self.db, session["session_id"]))
        finally:
