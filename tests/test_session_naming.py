...
class SessionNamingTests(unittest.TestCase):
    ...
    def test_standard_session_is_created_only_after_valid_name(self):
        ...
        self.assertTrue(_m_message_commands.handle_pending_input(self.db, "token", "chat", self.session, "  Project   Alpha  ", operation_id=42, memory_service=make_test_memory_service()))
    ...
    def test_invalid_name_reprompts_without_creating_session(self):
        _m_session_naming.start_session_name_input(self.db, "token", "chat", self.session)
        self.assertTrue(_m_message_commands.handle_pending_input(self.db, "token", "chat", self.session, "/bad", operation_id=43, memory_service=make_test_memory_service()))
        self.assertEqual(len(_m_panel_callback_routes.list_sessions(self.db, "chat")), 1)
    ...
    def test_cancel_leaves_no_empty_session_or_pending_character(self):
        ...
        _m_session_naming.start_session_name_input(self.db, "token", "chat", self.session)
        self.assertTrue(_m_message_commands.handle_pending_input(self.db, "token", "chat", self.session, "/cancel", operation_id=44, memory_service=make_test_memory_service()))
        self.assertEqual(len(_m_panel_callback_routes.list_sessions(self.db, "chat")), 1)
    ...
    def test_cancel_with_bot_mention_leaves_no_empty_session(self):
        ...
        self.assertTrue(_m_message_commands.handle_pending_input(self.db, "token", "chat|topic:7", self.session, "/cancel@SillyTavernPunzmeBot", operation_id=47, memory_service=make_test_memory_service()))
    ...
    def test_character_chain_applies_character_after_name(self):
        ...
        try:
            _m_message_commands.handle_pending_input(self.db, "token", "chat", self.session, "Chosen Story", operation_id=45, memory_service=make_test_memory_service())
        finally:
    ...
    def test_group_session_waits_for_name_then_opens_character_stage(self):
        ...
        try:
            ...
            _m_message_commands.handle_pending_input(self.db, "token", chat_id, session, "Mystery Team", operation_id=46, memory_service=make_test_memory_service())
        finally:
