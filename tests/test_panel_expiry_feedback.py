...
class PanelExpiryFeedbackTests(SettingsTestCase):
    ...
    def test_queued_expired_panel_sends_visible_feedback_and_purges_binding(self):
        ...
        self.db.execute(
            "UPDATE panel_sessions SET expires_at=? WHERE chat_id=? AND message_id=?", (time.time() - 1, "chat", "501")
        )
        self.db.commit()
        sent, answered, closed = [], [], []
        original_send, original_answer, original_close = (
            _m_callback_dispatch.send_text,
            _m_callback_dispatch.answer_callback,
            _m_callback_dispatch.close_panel_message,
        )
    ...
    def test_nonqueued_expired_panel_keeps_callback_toast_behavior(self):
        ...
        self.assertEqual(closed, [callback])
        self.assertIsNone(
            self.db.execute("SELECT 1 FROM panel_sessions WHERE chat_id=? AND message_id=?", ("chat", "502")).fetchone()
        )
