...
def _image_bytes_from_response(payload: dict, spec: dict) -> tuple[bytes, str]:
    ...
    with strict_urlopen(urllib.request.Request(url, headers={"Accept": "image/*"}), timeout=120) as response:  # noqa: S310 -- strict_urlopen validates scheme and host
...
def generate_image(selection: str, prompt: str, size: str = IMAGE_DEFAULT_SIZE) -> tuple[bytes, str, str]:
    ...
    request = urllib.request.Request(  # noqa: S310 -- strict_urlopen validates scheme and host
        endpoint, data=json.dumps(body).encode(), headers=_image_headers(spec), method="POST"
    )
...
def handle_imagine_prompt(
    token: str, chat_id: str, prompt: str, selection: str = "", size: str = IMAGE_DEFAULT_SIZE
) -> None:
