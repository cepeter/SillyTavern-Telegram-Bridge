...
def card_fields(card: dict) -> dict[str, str]:
    ...
    if not isinstance(alternate, list):
        alternate = []
    fields["alternate_greetings"] = json.dumps([str(value)[:_config.CARD_FIELD_MAX_CHARS] for value in alternate[:20]], ensure_ascii=False)
    return fields
...
def build_world_info(world_names: str | list[str], context: str, fields: dict[str, str], user_name: str = _config.DEFAULT_USER_NAME) -> str:
