...
def send_group_menu(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], message_id: int | None = None, *, request_context) -> None:
...
def apply_group_setup_character(db: sqlite3.Connection, token: str, callback: dict, answer_callback, chat_id: str, message: dict, session_id: str, operation_id: int | str | None, filename: str, character_name: str, *, request_context) -> bool:
    ...
    return True
def send_group_character_menu(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], action: str, message_id: int | None = None, page: int = 0, *, request_context) -> None:
    state = group_state(db, chat_id, session["session_id"])
...
def send_group_speaker_menu(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], message_id: int | None = None, *, request_context) -> None:
    state = group_state(db, chat_id, session["session_id"])
    rows = [[{"text": panel_label(label), "callback_data": f"groupchars:speak:{dynamic_callback_token('group_character', filename, chat_id, db=request_context.db)}"}] for filename, label in zip(state["members"], group_member_labels(state["members"]))]
    rows.append([{"text": "⬅️ Back", "callback_data": "group:menu"}, {"text": "❌ Close", "callback_data": "group:close"}])
...
def send_group_mode_menu(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], message_id: int | None = None, *, request_context) -> None:
    state = group_state(db, chat_id, session["session_id"])
...
def handle_group_panel_callback(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], data: str, message: dict, operation_id: int | str | None = None, sender_id: str = "", *, request_context) -> None:
    ...
    if data == "group:new_session":
        ...
        send_group_menu( db, token, chat_id, session, message_id, request_context=request_context)
    elif data == "group:add":
        send_group_character_menu( db, token, chat_id, session, "add", message_id, request_context=request_context)
    elif data == "group:remove":
        send_group_character_menu( db, token, chat_id, session, "remove", message_id, request_context=request_context)
    elif data == "group:speak":
        send_group_speaker_menu( db, token, chat_id, session, message_id, request_context=request_context)
    elif data == "group:mode":
        send_group_mode_menu( db, token, chat_id, session, message_id, request_context=request_context)
        ...
        send_group_character_menu( db, token, chat_id, session, action, message_id, int(page), request_context=request_context)
