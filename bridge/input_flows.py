...
def handle_inline_text_action(db, token: str, api_key: str, chat_id: str, session: dict, fields: dict, action: str, value: str, operation_id: int | None = None, *, memory_service=None) -> bool:
    ...
    return _handle_text_action_input(db, token, api_key, chat_id, session, fields, value, state, operation_id, memory_service=memory_service)


def _handle_text_action_input(db, token: str, api_key: str, chat_id: str, session: dict, fields: dict, stripped: str, state: dict, operation_id: int | None, *, memory_service=None) -> bool:
    meta_key = f"text_action_input:{chat_id}"
    ...
    try:
        if action == "edit":
            edit_last_user(db, token, api_key, session, fields, chat_id, value[:12000], operation_id=operation_id, memory_service=memory_service)
...
def handle_pending_input(db: sqlite3.Connection, token: str, chat_id: str, session: dict, stripped: str, api_key: str = "", fields: dict | None = None, operation_id: int | None = None, *, memory_service=None) -> bool:
    ...
    if text_action:
        ...
        return _handle_text_action_input(db, token, api_key, chat_id, session, action_fields, stripped, text_action, operation_id, memory_service=memory_service)
