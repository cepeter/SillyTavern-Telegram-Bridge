...
def handle_generation_settings(db: sqlite3.Connection, token: str, chat_id: str, session_id: str, command_text: str) -> None:
    if command_text.casefold() == "/settings":
        send_text(token, chat_id, f"Generation settings for this session:\n{format_generation_settings(get_generation_settings(db, chat_id, session_id))}\n\nSet: /settings <name> <value>\nReset: /settings reset")
        return
    argument = command_text.split(None, 1)[1].strip() if len(command_text.split(None, 1)) > 1 else ""
    if argument.casefold() == "reset":
        update_generation_settings(db, chat_id, session_id, **GENERATION_DEFAULTS)
        send_text(token, chat_id, "Generation settings reset for this session.")
        return
    parts = argument.split(None, 1)
    if len(parts) != 2:
        send_text(token, chat_id, "Use /settings <name> <value>. Example: /settings temperature 0.7")
        return
    try:
        key, value = parse_generation_setting(parts[0], parts[1].strip())
        settings = update_generation_settings(db, chat_id, session_id, **{key: value})
        send_text(token, chat_id, f"Updated {key} for this session.\n{format_generation_settings(settings)}")
    except ValueError as exc:
        send_text(token, chat_id, f"Invalid setting: {exc}")
