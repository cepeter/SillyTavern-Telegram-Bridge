...
def start_session_name_input(
    ...
    *,
    ...
    state["prompt_message_ids"] = send_text(
        token, chat_id, f"{prompt} (1–{_session_titles.SESSION_TITLE_MAX_CHARS} characters). Send /cancel to cancel."
    )
...
def handle_session_name_input(
    ...
    *,
    ...
    _cancel_pending(db, token, chat_id, meta_key, state)
    suffix = (
        f"\nCharacter: {pending_character.get('character_name') or Path(pending_character['character_file']).stem}"
        if pending_character
        else ""
    )
