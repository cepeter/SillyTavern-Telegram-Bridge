...
def response_language_instruction(value: str | None) -> str:
    ...
    if language == "auto":
        return (
            "Reply in the same language as the user's latest message. Keep the "
            "language consistent unless the user explicitly requests a change."
        )
    ...
    return (
        ...
        " ("
        f"""{language}"""
        "). You MUST write all visible response text in "
        f"""{label}"""
        ". Do not switch because the character card, persona, memory, World Info, "
        "examples, or conversation history uses another language. Only switch "
        "when the user's latest message explicitly asks for a different response "
        "language."
    )
...
def send_language_menu(
    ...
    *,
    ...
    text = (
        ...
        f"""{page_text}"""
        "\nCurrent: "
        f"""{response_language_label(current)}"""
