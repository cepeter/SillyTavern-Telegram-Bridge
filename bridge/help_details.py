...
def help_markup(category: str | None = None, command_index: int | None = None, page: int = 0) -> dict:
    ...
    if category is not None and category in HELP_CATEGORIES and command_index is not None:
        return {
            "inline_keyboard": [
                [
                    {"text": "⬅️ Back", "callback_data": f"help:{category}"},
    ...
    if category is not None and category in HELP_CATEGORIES:
        ...
        rows = [
            [{"text": command, "callback_data": f"help:cmd:{category}:{offset + index}"}]
            for index, (command, _summary) in enumerate(options)
        ]
        navigation = []
        if current_page > 0:
        ...
        rows.append(
            [
                {"text": "⬅️ Categories", "callback_data": "help:menu"},
                {"text": "❌ Close", "callback_data": "help:close"},
            ]
        )
    ...
    return {
        "inline_keyboard": [
            [
                {"text": "💬 Start & Sessions", "callback_data": "help:basic"},
                {"text": "🎭 Characters & Prompts", "callback_data": "help:characters"},
            ],
            [
                {"text": "⚙️ Replies & Settings", "callback_data": "help:generation"},
                {"text": "🧠 Memory & Files", "callback_data": "help:memory_rag"},
            ],
...
def help_text(category: str | None = None, command_index: int | None = None, page: int = 0) -> str:
    ...
    return (
        "Help — SillyTavern Telegram Bridge\n\n"
