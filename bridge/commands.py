...
def handle_macro_command(db: sqlite3.Connection, token: str, chat_id: str, session: dict[str, str], fields: dict[str, str], command_text: str) -> None:
    ...
    if action.casefold() == "note" and argument.strip():
        ...
        db.commit()
        send_text(token, chat_id, "STscript reset applied to this session.")
    else:
        send_text(token, chat_id, "Safe STscript commands: /stscript note <text> or /stscript reset")


def handle_preset_command(db: sqlite3.Connection, token: str, chat_id: str, session_id: str, command_text: str) -> None:
    parts = command_text.split(None, 2)
    action = parts[1].casefold() if len(parts) > 1 else "list"
    if action == "list":
        names = preset_names(db, chat_id)
        send_text(token, chat_id, "Presets: " + (", ".join(names) if names else "none") + "\nUse /preset save <name>, /preset use <name>, or /preset delete <name>.")
        return
    if len(parts) < 3:
        send_text(token, chat_id, "Use /preset save <name>, /preset use <name>, or /preset delete <name>.")
        return
    name = parts[2].strip()[:64]
    if not re.fullmatch(r"[A-Za-z0-9_-]+", name):
        send_text(token, chat_id, "Preset name must use letters, numbers, hyphens, or underscores.")
        return
    if action == "save":
        save_generation_preset(db, chat_id, name, get_generation_settings(db, chat_id, session_id))
        send_text(token, chat_id, f"Preset saved: {name}")
    elif action == "use":
